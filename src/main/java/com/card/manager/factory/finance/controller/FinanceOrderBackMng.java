...
@Controller
@RequestMapping("/admin/finance/orderBackMng")
public class FinanceOrderBackMng extends BaseController {
	...
	public ModelAndView list(HttpServletRequest req, HttpServletResponse resp) {
		...
		return forword("finance/back/list", context);
	}
	...
	public ModelAndView toShow(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			return forword("finance/back/show", context);
		} catch (Exception e) {
	...
	public ModelAndView toAudit(HttpServletRequest req, HttpServletResponse resp) {
		Map<String, Object> context = getRootMap();
		StaffEntity opt = SessionUtils.getOperator(req);
		context.put(OPT, opt);
		try {
			...
			return forword("finance/back/audit", context);
		} catch (Exception e) {
			context.put(ERROR, e.getMessage());
			return forword(ERROR, context);
		}
	...
	public void audit(HttpServletRequest req, HttpServletResponse resp) {
		StaffEntity staffEntity = SessionUtils.getOperator(req);
		try {
			String orderId = req.getParameter("orderId");
			String payNo = req.getParameter("payNo");
			if ("".equals(orderId) || "".equals(payNo)) {
				sendFailureMessage(resp, "信息获取不全，请重试！");
				return;
			}
			orderService.auditOrderBack(orderId,payNo,staffEntity);
		} catch (Exception e) {
