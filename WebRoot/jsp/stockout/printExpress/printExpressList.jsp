			...
			<div class="row">
				<div class="col-lg-12">
					<ol class="breadcrumb">
						...
						<li class="active"><i class="fa fa-user-md fa-fw"></i> 打印快递单</li>
					</ol>
				</div>
			</div>
			<div class="row">
					...
					<div class="panel panel-default" >
						...
						<div class="panel-body">
							<c:if test="${privilege>=2}">
								...
								<button type="button" class="btn btn-primary" onclick="printMerchBill()">打印商品清单</button>
								<button type="button" class="btn btn-primary" onclick="printByInstructs()">指示单打印快递单</button>
								<button type="button" class="btn btn-primary" onclick="printMerchBillByInstructs()">指示单打印商品清单</button>
							</c:if>
...
<script type="text/javascript">
	...
	var options = {
				queryForm : ".query",
				url :  "${wmsUrl}/admin/stockOut/printExpress/dataList.shtml",
				numPerPage:"50",
				currentPage:"",
				index:"1",
	...
	function printByInstructs(){
		var instructions = $("#instructions").val();
		
		if(instructions == null||""==instructions){
			$.zzComfirm.alertError("没有指示单信息");
	    	return;
		}
		var left1 = 300;
		var top1 = 200;
		window.open('${wmsUrl}/admin/stockOut/printExpress/toPrintByInstructions.shtml?instructions='+instructions+'&flag=0', "", "width=750,height=700,resizable=yes,location=no,scrollbars=yes,left=" + left1.toString() + ",top=" + top1.toString());
	...
	function printExpress(){
	    ...
	    $("[name='check']:checked").each(function(i){ 
	        valArr[i] = $(this).val(); 
	        insArr[i] = $(this).parent().next().next().html();
	    }); 
	    
	    if(valArr.length > 300){
	    	$.zzComfirm.alertError("数据过多，请使用按照【指示单打印快递单】");
	    	return;
	    }
	    ...
	    if(insArr == '' || insArr[0] == undefined){
	    	$.zzComfirm.alertError("指示号为空");
	    	return;
	    }
	    for(var i=1;i<insArr.length;i++){
	    	if(insArr[0] != insArr[i]){
	...
	function printMerchBill(){
	    ...
	    $("[name='check']:checked").each(function(i){ 
	        valArr[i] = $(this).val(); 
	        insArr[i] = $(this).parent().next().next().html();
	        shopNameArr[i] = $(this).parent().siblings("#shopName").html();
	    }); 
	    var orderIds = valArr.join(',');//转换为逗号隔开的字符串 
	    
	    if(valArr.length > 300){
	    	$.zzComfirm.alertError("数据过多，请使用按照【指示单打印清单】");
	    	return;
	    }
		...
		for(var i = 0;i<shopNameArr.length;i++){
			if(shopNameArr[i]=="天猫国际海外直营"){
				 flag = false;
			}
		}
		if(!flag){
			for(var j = 0;j<shopNameArr.length;j++){
				if(shopNameArr[0]!=shopNameArr[j]){
					$.zzComfirm.alertError('天猫国际海外直营请单独打印');  
					return ;
				}
		 	}
		}
		if(valArr!='' && insArr!=""){
			var left1 = 300;
			var top1 = 200;
			window.open('${wmsUrl}/admin/stockOut/printExpress/toPrint.shtml?orderIds='+orderIds+'&flag=1', "", "width=1200,height=700,resizable=yes,location=no,scrollbars=yes,left=" + left1.toString() + ",top=" + top1.toString());
		}
	}
	
	function printMerchBillByInstructs(){
		var instructions = $("#instructions").val();
		...
		var left1 = 300;
		var top1 = 200;
		window.open('${wmsUrl}/admin/stockOut/printExpress/toPrintByInstructions.shtml?instructions='+instructions+'&flag=1', "", "width=750,height=700,resizable=yes,location=no,scrollbars=yes,left=" + left1.toString() + ",top=" + top1.toString());
