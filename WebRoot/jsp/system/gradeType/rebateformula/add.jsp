       	...
       	<form class="form-horizontal" role="form" id="form" style="margin-top:20px;">
			<div class="list-item">
				...
				<div class="col-xs-9 item-right">
					<select class="form-control" name="gradeTypeId" id="gradeTypeId">
                   	  <c:forEach var="gradeType" items="${gradeTypeList}">
                   	  	<option value="${gradeType.id}">${gradeType.name}</option>
                   	  </c:forEach>
	                </select>
	                <div class="item-content">
		             	（分机类型里去掉了区域中心，因为区域中心为标准返佣）
		            </div>
				</div>
			</div>
			<div class="list-item">
				<div class="col-xs-3 item-left">返佣公式<font style="color:red">*</font> </div>
					<div class="col-xs-9 item-right">
						<input type="text" class="form-control" name="formula" id="formula" value = "rebate">
		            	<div class="item-content">
...
// 			 }
			 ...
			 obj.formula = formula;
			 $.ajax({
				 url:"${wmsUrl}/admin/system/gradeType/typerebate/add.shtml",
				 type:'post',
