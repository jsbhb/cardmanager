...
var options = {
			queryForm : ".query",
			url :  "${wmsUrl}/admin/goods/goodsRebateMng/dataList.shtml",
			numPerPage:"20",
			currentPage:"",
			index:"1",
...
function rebuildTable(data){
	...
	for (var i = 0; i < list.length; i++) {
		...
		str += "</td><td>" + (list[i].first == null ? "" : list[i].first);
...
function toShow(id){
	var index = layer.open({
		  title:"设置返佣",		
		  type: 2,
		  content: '${wmsUrl}/admin/goods/goodsRebateMng/toShow.shtml?id='+id,
		  maxmin: true
		});
		layer.full(index);
}
