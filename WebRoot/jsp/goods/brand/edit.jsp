	...
	<section class="content-iframe">
       	<form class="form-horizontal" role="form" id="brandForm" style="margin-top:20px;">
			...
			<div class="list-item">
				...
				<div class="col-xs-9 item-right">
           		    <input type="hidden" class="form-control" name="id" value="${brand.id}">
					<input type="hidden" class="form-control" name="brandId" readonly value="${brand.brandId}">
					<input type="text" class="form-control" id="brand" name="brand" readonly value="${brand.brand}">
	            	<div class="item-content">
			...
			<div class="list-item">
				...
				<div class="col-xs-9 item-right">
					<input type="text" class="form-control" id="brandNameEn" name="brandNameEn" value="${brand.brandNameEn}" onkeyup="joinName()">
	            	<div class="item-content">
	             		（请输入数字或英文，限0-20字）
	             	</div>
				</div>
			...
			<div class="list-item">
				...
				<div class="col-xs-9 item-right">
					<input type="text" class="form-control" id="brandNameCn" name="brandNameCn" value="${brand.brandNameCn}"onkeyup="joinName()">
	            	<div class="item-content">
	             		（请输入数字或汉字，限0-20字）
	             	</div>
				</div>
			...
			<div class="list-item">
	      	...
	      	<div class="list-item">
				...
				<div class="col-xs-9 item-right">
					<textarea class="form-control" name="brandSynopsis">${brand.brandSynopsis}</textarea>
             		<div class="item-content">
		             	（请输入品牌简介）
		            </div>
				</div>
			...
			<div class="list-item">
				...
				<div class="col-xs-9 item-right addContent">
					<c:choose>
					   <c:when test="${brand.brandLogo != null && brand.brandLogo != ''}">
								...
								<input value="${brand.brandLogo}" type="hidden" name="brandLogo" id="brandLogo">
							</div>
					   </c:when>
					   <c:otherwise>
...
//	      live: 'disabled',
	...
	//点击上传图片
		$('.item-right').on('change','.item-img input[type=file]',function(){
			...
			$.ajaxFileUpload({
				url : '${wmsUrl}/admin/uploadFileWithType.shtml?type=goodsBrand&key='+"${brand.brandId}", //你处理上传文件的服务端
				secureuri : false,
				fileElementId : "pic",
				dataType : 'json',
				success : function(data) {
