...
// 				url:"${wmsUrl}/admin/goods/propertyMng/queryPropertyValueById.shtml?id="+id,
	  ...
	  function checkTableInfo(){
		...
		function setGoodsInfo(data) {
			var typeSelect = $("#type");
			typeSelect.empty();
			if (data.data[0].goods.type == 0) {
				typeSelect.append("<option selected='selected' value='0'>跨境商品</option>");
				typeSelect.append("<option value='2'>一般贸易商品</option>");
			} else if (data.data[0].goods.type == 2) {
				typeSelect.append("<option value='0'>跨境商品</option>");
				typeSelect.append("<option selected='selected' value='2'>一般贸易商品</option>");
			}
			$("#type").attr("disabled", true);
			$("#brand").val(data.data[0].goods.brandName);
			$("#brandId").val(data.data[0].goods.brandId);
			$("#brand").attr("disabled", true);
			
			var firstSelect = $("#firstCatalogId");
			firstSelect.empty();
			firstSelect.append("<option selected='selected' value="+data.data[0].goods.firstCategory+">"+data.data[0].goods.firstCategoryName+"</option>");
			$("#firstCatalogId").attr("disabled", true);
			var secondSelect = $("#secondCatalogId");
			secondSelect.empty();
			secondSelect.append("<option selected='selected' value="+data.data[0].goods.secondCategory+">"+data.data[0].goods.secondCategoryName+"</option>");
			$("#secondCatalogId").attr("disabled", true);
			var thirdSelect = $("#thirdCatalogId");
			thirdSelect.empty();
			thirdSelect.append("<option selected='selected' value="+data.data[0].goods.thirdCategory+">"+data.data[0].goods.thirdCategoryName+"</option>");
			$("#thirdCatalogId").attr("disabled", true);
			
			$("#goodsName").val(data.data[0].goods.goodsName);
			$("#goodsName").attr("disabled", true);
			$("#goodsId").val(data.data[0].goods.goodsId);
			$("#description").val(data.data[0].goods.description);
			$("#description").attr("disabled", true);
			$("#origin").val(data.data[0].goods.origin);
			$("#origin").attr("disabled", true);
			$("#hscode").val(data.data[0].goods.hscode);
			$("#hscode").attr("disabled", true);
			
			var imgHt = "";
			for (var i=0;i<data.data[0].goods.goodsFileList.length;i++) {
				imgHt = imgHt+ '<div class="item-img choose" id="'+(i+1)+'" data-id="'+(i+1)+'">';
				imgHt = imgHt+ '<img src="'+data.data[0].goods.goodsFileList[i].path+'"><div class="bgColor"><i class="fa fa-search fa-fw"></i></div>';
				imgHt = imgHt+ '<input type="hidden" value='+data.data[0].goods.goodsFileList[i].path+' name="picPath" id="picPath'+(i+1)+'"></div>';
			}
			$(".addContent").html(imgHt);
			
			$("#encode").val(data.data[0].specsList[0].encode);
			$("#encode").attr("disabled", true);
			$("#specsId").val(data.data[0].specsList[0].specsId);
			$("#specsTpId").val(data.data[0].specsTpList[0].specsTpId);
			$("#encodeExist").val("isExist");
			$("#unit").val(data.data[0].specsList[0].unit);
			$("#unit").attr("disabled", true);
			$("#weight").val(data.data[0].specsList[0].weight);
			$("#weight").attr("disabled", true);
			$("#specsGoodsName").val(data.data[0].specsList[0].specsGoodsName);
			$("#specsGoodsName").attr("disabled", true);
			$("#specsDescription").val(data.data[0].specsList[0].description);
			$("#specsDescription").attr("disabled", true);
			$("#conversion").val(data.data[0].specsList[0].conversion);
			$("#conversion").attr("disabled", true);
			
			var specsHt = "";
			specsHt = specsHt+ '<div class="list-item col-sm-6 col-sm-offset-3"><div class="list-all">';
			for (var i=0;i<data.data[0].specsList.length;i++) {
				for (var j=0;j<data.data[0].specsList[i].itemSpecsList.length;j++) {
					...
					specsHt = specsHt+ '<option value="'+data.data[0].specsList[i].itemSpecsList[j].skId+'" selected="selected">'+data.data[0].specsList[i].itemSpecsList[j].skV+'</option>';
					...
					specsHt = specsHt+ '<option value="'+data.data[0].specsList[i].itemSpecsList[j].svId+'">'+data.data[0].specsList[i].itemSpecsList[j].svV+'</option>';
			...
			UE.getEditor('editor').execCommand('insertHtml', data.data[0].goods.detailPath);
			rebuildSupplierTable();
			
			$("#bindSupplierTable tbody").html("");
			var str = "";
			var list = data.data[0].itemList;
			if (list == null || list.length == 0) {
